...
class ShadowModel(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.pool(F.relu(self.conv3(x)))  # Output size: (batch, 128, 3, 3)
        x = x.view(x.size(0), -1)  # Flatten
        x = F.relu(self.fc1(x))
        x = self.dropout(x)
        x = F.relu(self.fc2(x))
        x = self.dropout(x)
        x = self.fc3(x)  # No activation, as it's used with CrossEntropyLoss
        return x
...
def load_datasets_from_s3(args, dataset_prefix, local_dir="./datasets"):
    ...
    for obj in response["Contents"]:
        key = obj["Key"]
        if key.endswith(".json"):  # Ignore JSON mapping file
            continue
...
def load_fashionProd_from_s3(args, local_dir="./datasets"):
    ...
    class_mapping_path = os.path.join(local_dir, "class_mapping.json")
    s3_mapping_key = f"fashion-product-images-processed/class_mapping.json"
    ...
    for split_name in sorted(split_names):  # Sorting ensures consistency
        ...
        datasets_dict[split_name] = FashionProductDataset(local_split_path, class_mapping, transform=transform)
