...
from sklearn.metrics import (
    accuracy_score, precision_score, recall_score,  classification_report, roc_auc_score, confusion_matrix, f1_score
)
...
def compute_attack_metrics(attack_results, max_classes=5):
    """
    Computes membership inference attack metrics with quick debugging for large data.

    Args:
        attack_results (dict): Dictionary containing attack predictions for each class.
        max_classes (int): Limit the number of classes processed for faster debugging.
    ...
    for cls, results in attack_results.items():
        # Limit the number of classes processed for debugging
        if processed_classes >= max_classes:
            break

        # Extract predictions safely
        member_preds = results.get("member_preds", None)
        nonmember_preds = results.get("nonmember_preds", None)

         # Convert predictions to NumPy arrays
        member_preds = np.array([member_preds]) if np.isscalar(member_preds) else np.array(member_preds)
        nonmember_preds = np.array([nonmember_preds]) if np.isscalar(nonmember_preds) else np.array(nonmember_preds)

       # Ensure member_preds is a 1D NumPy array
        if member_preds is None:
            member_preds = np.array([])
        else:
            member_preds = np.array(member_preds).reshape(-1)

        # Ensure nonmember_preds is a 1D NumPy array
        if nonmember_preds is None:
            nonmember_preds = np.array([])
        else:
            nonmember_preds = np.array(nonmember_preds).reshape(-1)

       
        # Skip if both arrays are empty
        if member_preds.size == 0 and nonmember_preds.size == 0:
            print(f"⚠️ Skipping class {cls} due to missing predictions.")
            metrics[cls] = {metric: 0.0 for metric in ["Accuracy", "TPR", "TNR", "FPR", "FNR",
                                                       "Advantage", "AUC"]}
            continue
        ...
        # Concatenate predictions and labels for computation
        all_preds = np.concatenate((member_preds, nonmember_preds))
        all_labels = np.concatenate((member_labels, nonmember_labels))

        # Accuracy
        accuracy = accuracy_score(all_labels, np.round(all_preds))  # Rounded to get binary classification
        ...
        tp = np.sum((all_labels == 1) & (np.round(all_preds) == 1))
        tn = np.sum((all_labels == 0) & (np.round(all_preds) == 0))
        fp = np.sum((all_labels == 0) & (np.round(all_preds) == 1))
        fn = np.sum((all_labels == 1) & (np.round(all_preds) == 0))
        ...
        except ValueError:  # In case all predictions are the same
            auc = np.nan
