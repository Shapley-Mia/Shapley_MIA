...
def compute_attack_metrics(attack_results):
    ...
    for cls, results in attack_results.items():
        ...
        TP = np.sum((member_predictions == 1.0) & (member_labels == 1.0))  # Correctly predicted members
        FN = np.sum((member_predictions == 0.0) & (member_labels == 1.0))  # Members misclassified as non-members
        TN = np.sum((nonmember_predictions == 0.0) & (nonmember_labels == 0.0))  # Correctly predicted non-members
        FP = np.sum((nonmember_predictions == 1.0) & (nonmember_labels == 0.0))  # Non-members misclassified as members
    ...
    avg_metrics = {key: np.mean([m[key] for m in metrics.values()]) for key in metrics[next(iter(metrics))]}
...
def run_attack_multiple_checkpoints(args, prefix, type_node="nodes"):
    ...
    # ✅ Use absolute path
    local_results_file = os.path.join(output_dir, f"{type_node}_attack_results_new.json")

    print(f"✅ Writing results to {local_results_file}")
    with open(local_results_file, "w") as f:
        json.dump(all_results, f, indent=4)

    output_s3_path = f"s3://{args.results_bucket}/{args.results_prefix}/{type_node}_attack_results_new.json"
    s3_client.upload_file(local_results_file, args.results_bucket, f"{args.results_prefix}/{type_node}_attack_results_new.json")
